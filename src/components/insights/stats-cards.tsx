...
export function StatsCards({ entries }: StatsCardsProps) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 stagger-children">
      {items.map((item) => (
        <div key={item.label} className="rounded-lg border border-border/60 bg-card/30 px-4 py-4">
          <p className="text-[11px] uppercase tracking-[0.12em] text-muted-foreground/60 mb-2">
            {item.label}
          </p>
          <p className="font-display text-2xl tracking-tight capitalize">{item.value}</p>
          {item.sub && <p className="text-[11px] text-muted-foreground/50 mt-0.5">{item.sub}</p>}
        </div>
      ))}
    </div>
  );
